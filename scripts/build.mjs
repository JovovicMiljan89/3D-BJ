...
import { validateContent, THEMES } from "./lib/validate.mjs";
...
// Both overridable only so the e2e suite can serve a second, teget-themed
// build side by side; production builds never set them.
const DIST_DIR = process.env.BUILD_DIST_DIR ? path.resolve(process.env.BUILD_DIST_DIR) : path.join(ROOT, "dist");
const THEME_OVERRIDE = process.env.BUILD_THEME || "";
if (THEME_OVERRIDE && !THEMES.includes(THEME_OVERRIDE)) {
  console.error(`[build] BUILD_THEME must be one of: ${THEMES.join(", ")}`);
  process.exit(1);
}
...
// Image fields that have a black & white (-bw) variant for the crnobela theme
// (generated by scripts/make-bw-assets.mjs, committed next to the originals).
const BW_IMAGE_FIELDS = [
  ["brand", "logo"],
  ["brand", "logoFull"],
  ["brand", "mark"],
  ["brand", "favicon"],
...
function applyTheme(data) {
  const name = THEME_OVERRIDE || data.theme || "crnobela";
  if (name === "crnobela") {
    for (const [section, field] of BW_IMAGE_FIELDS) {
      const original = data[section][field];
      const bw = bwPath(original);
      if (fs.existsSync(path.join(ROOT, bw.replace(/^\//, "")))) {
        data[section][field] = bw;
      } else {
        console.warn(`[build] WARNING: no black & white variant ${bw} — using ${original} (run: node scripts/make-bw-assets.mjs).`);
      }
    }
  }
  data.theme = {
    name,
    color: themeBackground(name),
    grayscalePhotos: name === "crnobela" && data.grayscalePhotos !== false,
  };
}
