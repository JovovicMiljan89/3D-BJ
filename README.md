...
| Color theme (crnobela / teget) | `theme` in `content/site.json`; palettes are the two `:root[data-theme=…]` token blocks at the top of `css/styles.css`. B&W logo/icon/OG variants (`*-bw.*`) are generated by `node scripts/make-bw-assets.mjs` (needs the e2e Playwright install) and committed. |
